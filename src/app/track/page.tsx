...
export default function TrackPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 pb-32">
      ...
      <div className={`px-4 pt-5 ${order && !['pending','rejected','completed'].includes(order.status) ? 'pb-40' : 'pb-24'}`}>
        {loading ? (
      ...
      {/* Fixed Driver Bar — shows from 'preparing' onward, positioned above the bottom nav */}
      {order && !['pending','rejected','completed'].includes(order.status) && (
        <div className="fixed bottom-16 left-0 right-0 z-40 bg-white dark:bg-slate-800 border-t border-gray-100 dark:border-slate-700 shadow-xl" dir="ltr">
          <div className="flex items-center gap-3 px-4 py-2 max-w-lg mx-auto">
            <img src="/driver-cartoon.svg" alt="" className="w-14 h-14 flex-shrink-0" />
            {order.driver_name ? (
              /* driver assigned — show name + WhatsApp */
              <>
                <div className="flex-1 text-right" dir="rtl">
                  <p className="font-black text-base leading-tight text-red-500 dark:text-red-400">{order.driver_name}</p>
                  <p className="text-blue-600 dark:text-blue-400 text-xs font-bold">سيوصل طلبك</p>
                </div>
                {order.driver_phone && (
                  <a
                    ...
                    className="flex flex-col items-center justify-center gap-0.5 rounded-xl px-3 py-2 flex-shrink-0 active:scale-90 transition-all"
                  ...
                  >
                    ...
                    <span className="text-white text-xs font-bold">واتساب</span>
                  </a>
                )}
              </>
            ) : (
              ...
              <div className="flex-1 text-right" dir="rtl">
                <div className="flex items-center justify-end gap-2">
                  <p className="text-gray-500 dark:text-slate-400 text-sm font-bold">جاري تعيين سائق...</p>
                  <div className="w-4 h-4 border-2 border-orange-400 border-t-transparent rounded-full animate-spin flex-shrink-0" />
                </div>
                <p className="text-gray-400 dark:text-slate-500 text-xs mt-0.5">سيظهر معلومات السائق قريباً</p>
              </div>
            )}
          </div>
        </div>
