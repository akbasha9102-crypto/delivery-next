      ...
      <div className="px-4 pt-5 pb-24">
        {loading ? (
          ...
          <div className="space-y-4 max-w-lg mx-auto">
            ...
            {/* Driver Info — shows from 'preparing' onward */}
            {!['pending', 'rejected', 'completed'].includes(order.status) && (
              <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-gray-100 dark:border-slate-700" dir="rtl">
                <h3 className="font-bold text-gray-900 dark:text-slate-100 mb-4">السائق</h3>
                {order.driver_name ? (
                  <div className="flex items-center gap-4">
                    <img src="/driver-cartoon.svg" alt="" className="w-14 h-14 flex-shrink-0" />
                    <div className="flex-1">
                      <p className="font-black text-lg text-gray-900 dark:text-white leading-tight">{order.driver_name}</p>
                    ...
                    {order.driver_phone && (
                      <a
                        ...
                        className="flex flex-col items-center gap-1 rounded-2xl px-4 py-3 active:scale-90 transition-all flex-shrink-0"
                      ...
                      >
                        ...
                        <span className="text-white text-xs font-bold">{order.driver_phone.replace(/\D/g, '')}</span>
                      </a>
                    )}
                  </div>
                ) : (
                  <div className="flex items-center gap-3">
                    <img src="/driver-cartoon.svg" alt="" className="w-12 h-12 flex-shrink-0 opacity-40" />
                    <div>
                      <div className="flex items-center gap-2">
                        <div className="w-4 h-4 border-2 border-orange-400 border-t-transparent rounded-full animate-spin" />
                        <p className="text-gray-500 dark:text-slate-400 text-sm font-bold">جاري تعيين سائق...</p>
                      </div>
                      <p className="text-gray-400 dark:text-slate-500 text-xs mt-1">سيظهر معلومات السائق قريباً</p>
                    </div>
                  </div>
                )}
              </div>
