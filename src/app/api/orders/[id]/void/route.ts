...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  // تنظيف إضافي best-effort: تعديل زبون معلّق على طلب أُلغي للتو يجب ألا يبقى
  // قابلاً للقبول لاحقاً (حراسة reviewEdit الوحيدة هي kitchen_ready_at IS NULL،
  // وطلب ملغى لا يزال يحقق هذا الشرط) — لا تُفشل الإلغاء الأساسي (نجح فعلاً)
  ...
  if (order.pending_edit_id) {
    try {
      ...
      console.error('تعذّر تنظيف pending_edit_id بعد إلغاء الطلب', orderId, err);
    }
  }
