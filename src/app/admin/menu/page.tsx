...
export default function MenuPage() {
  ...
  const isModalOpen = showAddCatSheet || showAddItemSheet || !!editItem || showReorder || !!editCatSheet;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#212121] pb-24 md:pb-0 md:mr-[70px]">
      ...
      {/* Reorder Modal */}
      {showReorder && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={() => setShowReorder(false)}>
          <div className="w-full max-w-lg bg-white dark:bg-slate-800 rounded-3xl max-h-[88vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-5 border-b border-gray-100 dark:border-slate-700">
      ...
      {/* Header */}
      <AdminHeader title="تعديل المنيو" />
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full flex items-center gap-4 bg-white dark:bg-slate-800 rounded-2xl p-5 border border-gray-100 dark:border-slate-700 active:scale-95 transition-all">
              <div className="w-14 h-14 rounded-2xl bg-gray-100 dark:bg-slate-700 flex items-center justify-center flex-shrink-0">
                <UtensilsCrossed size={26} className="text-gray-500 dark:text-slate-400" />
              </div>
              <div className="text-right flex-1">
                  ...
                  {section.fields.map(({ key, name }) => (
                    <div key={key} className="flex items-center justify-between gap-3 p-2 rounded-xl">
                      <button
                        ...
                        className={`w-9 h-9 rounded-xl flex-shrink-0 border-2 transition-all active:scale-90 ${activeColorPicker === key ? 'border-[var(--admin-accent-light-border)] scale-110 shadow-md dark:border-[var(--admin-accent-dark)]' : 'border-gray-300 dark:border-slate-500'}`}
