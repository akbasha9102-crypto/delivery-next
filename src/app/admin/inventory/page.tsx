        ...
        <div className="px-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 mb-3 scrollbar-none">
            ...
            <button onClick={() => setCatFilter(null)}
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold border transition-all active:scale-95 ${!catFilter ? 'bg-[var(--admin-accent-light-bg)] border-[var(--admin-accent-light-border)] text-white dark:bg-[var(--admin-accent-dark)] dark:border-[var(--admin-accent-dark)]' : 'bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-600 text-gray-500 dark:text-slate-400'}`}>
              الكل
            </button>
            {categories.map(c => {
                ...
                <button key={c} onClick={() => setCatFilter(catFilter === c ? null : c)}
                  className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold border transition-all active:scale-95 ${
                    colorStyle
                      ? 'text-black'
                      : selected
                        ? 'bg-[var(--admin-accent-light-bg)] border-[var(--admin-accent-light-border)] text-white dark:bg-[var(--admin-accent-dark)] dark:border-[var(--admin-accent-dark)]'
          ...
          {movements.length > 0 && movementCategories.length > 0 && (
            <div className="flex gap-2 overflow-x-auto pb-2 mb-3 scrollbar-none">
              <button onClick={() => setMovCatFilter(null)}
                className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold border transition-all active:scale-95 ${!movCatFilter ? 'bg-[var(--admin-accent-light-bg)] border-[var(--admin-accent-light-border)] text-white dark:bg-[var(--admin-accent-dark)] dark:border-[var(--admin-accent-dark)]' : 'bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-600 text-gray-500 dark:text-slate-400'}`}>
                الكل
              </button>
              {movementCategories.map(c => (
                <button key={c} onClick={() => setMovCatFilter(movCatFilter === c ? null : c)}
                  className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold border transition-all active:scale-95 ${movCatFilter === c ? 'bg-[var(--admin-accent-light-bg)] border-[var(--admin-accent-light-border)] text-white dark:bg-[var(--admin-accent-dark)] dark:border-[var(--admin-accent-dark)]' : 'bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-600 text-gray-500 dark:text-slate-400'}`}>
                  {c}
                </button>
              ))}
            </div>
                ...
                {(['IN', 'WASTE', 'ADJUSTMENT', 'RETURN', 'OUT_ORDER'] as MovementType[]).map(t => {
                  ...
                  return (
                    <button key={t} onClick={() => setMovType(t)}
                      className={`py-2.5 rounded-xl text-xs font-bold border transition-all active:scale-95 ${movType === t ? 'bg-[var(--admin-accent-light-bg)] border-[var(--admin-accent-light-border)] text-white dark:bg-[var(--admin-accent-dark)] dark:border-[var(--admin-accent-dark)]' : 'bg-gray-50 dark:bg-slate-700 border-gray-200 dark:border-slate-600 text-gray-600 dark:text-slate-300'}`}>
                      {cfg.label}
                    </button>
                  );
                })}
            ...
            <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-5 pt-6 pb-2">
              ...
              <div className="flex gap-1.5 flex-wrap mb-3 justify-end">
                <button onClick={() => selectPurchaseCatFilter(null)}
                  className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all active:scale-95 ${!purchaseCatFilter ? 'bg-[var(--admin-accent-light-bg)] border-[var(--admin-accent-light-border)] text-white dark:bg-[var(--admin-accent-dark)] dark:border-[var(--admin-accent-dark)]' : 'bg-gray-50 dark:bg-slate-700 border-gray-200 dark:border-slate-600 text-gray-500 dark:text-slate-400'}`}>
                  الكل
                </button>
                {categories.map(c => {
                  const dbCat = categoryRows.find(cr => cr.name === c);
              ...
              <p className="text-xs text-gray-400 dark:text-slate-500 text-right mb-1">
                ...
                {selectedPurchaseItem?.reorder_quantity ? <span className="text-[var(--admin-accent-light-bg)]"> — مقترحة حسب كمية إعادة الطلب</span> : ''}
              </p>
              ...
              <p className="text-xs text-gray-400 dark:text-slate-500 text-right mb-1">
                ...
                {selectedPurchaseItem && selectedPurchaseItem.cost_per_unit > 0 ? <span className="text-[var(--admin-accent-light-bg)]"> — مقترح حسب آخر سعر</span> : ''}
              </p>
          ...
          <div className="w-full max-w-lg bg-white dark:bg-slate-800 rounded-3xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain p-5">
              {!treeMaterial ? (
                ...
                <>
                  <p className="font-bold text-[var(--admin-accent-light-bg)] text-right mb-1">{treeMaterial.name}</p>
                  <p className="text-xs text-gray-400 dark:text-slate-500 text-right mb-4">وحدة المخزون: {treeMaterial.unit}</p>

                  {treeLoading ? (
                    <div className="flex justify-center py-6"><div className="w-8 h-8 border-4 border-[var(--admin-accent-light-border)] border-t-transparent rounded-full animate-spin" /></div>
                  ) : (
                    <>
                      {treeLinks.length === 0 && (
