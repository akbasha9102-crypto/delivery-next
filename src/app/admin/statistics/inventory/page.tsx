  ...
  return (
    ...
    <div className="min-h-screen pb-24 md:pb-0 md:mr-[70px]" style={{ backgroundColor: s.bg }}>
      ...
      <AdminHeader
        ...
        title={<span style={{ color: s.text }}>إحصائيات المخزون</span>}
        right={
      ...
      {categoryTabs.length > 0 && (
        <div className="flex gap-2 overflow-x-auto pb-2 px-4 mb-1 scrollbar-none">
          <button onClick={() => setCategoryTab(null)}
            className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold border transition-all active:scale-95 ${!activeCategoryTab ? 'bg-[var(--admin-accent-light-bg)] border-[var(--admin-accent-light-border)] text-white dark:bg-[var(--admin-accent-dark)] dark:border-[var(--admin-accent-dark)]' : ''}`}
            ...
            return (
              <button key={c} onClick={() => setCategoryTab(selected ? null : c)}
                className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold border transition-all active:scale-95 ${
                  colorStyle ? 'text-black' : selected ? 'bg-[var(--admin-accent-light-bg)] border-[var(--admin-accent-light-border)] text-white dark:bg-[var(--admin-accent-dark)] dark:border-[var(--admin-accent-dark)]' : ''
                }`}
