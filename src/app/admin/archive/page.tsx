...
function ArchivePage() {
  ...
  const fetchFeedbacks = useCallback(async () => {
    const { data } = await supabase
      .from('order_feedback')
      .select('*, orders(total_amount, delivery_address)')
      .order('created_at', { ascending: false })
      .limit(200);

    const enriched: Feedback[] = (data || []).map((f: any) => ({
      ...f,
        ...
        {loading ? (
          ...
          <div className="text-center mt-24">
            ...
            <p className="text-gray-400 dark:text-slate-500 font-medium">
              {feedbacks.length === 0 ? 'لا توجد ملاحظات بعد' : 'لا توجد نتائج'}
            </p>
          </div>
        ) : (
                ...
                <div className="p-4">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2">
                      {fb.type === 'complaint' ? (
                        <span className="flex items-center gap-1 text-xs font-bold text-red-500 bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded-full">
                          <AlertCircle size={11} /> شكوى
                        </span>
                      ) : (
                        <span className="flex items-center gap-1 text-xs font-bold text-blue-500 bg-blue-50 dark:bg-blue-900/20 px-2 py-1 rounded-full">
                          <MessageSquare size={11} /> ملاحظة
                        </span>
                      )}
                    </div>
                  ...
                  <p className="text-gray-700 dark:text-slate-300 text-sm leading-relaxed text-right mb-3 bg-gray-50 dark:bg-slate-700/50 rounded-xl px-3 py-2.5">
                    {fb.message}
                  </p>

                  <div className="flex justify-between items-center text-xs text-gray-400 dark:text-slate-500">
                    <span>
                      {new Date(fb.created_at).toLocaleDateString('ar-IQ', {
                        month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit',
                      })}
                    </span>
                    {fb.total_amount != null && (
                      <span className="text-green-500 font-bold">{fb.total_amount.toLocaleString()} د.ع</span>
                    )}
                  </div>
                </div>
              </div>
