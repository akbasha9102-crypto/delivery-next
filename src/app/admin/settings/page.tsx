...
export default function SettingsPage() {
  ...
  // زرّا "الموظفين" و"سجل التدقيق": لم يعودا مقفلين لباقة standard، القيد
  // الوحيد المتبقي عليهما هو دور الكاشير — لا علاقة لهذا بأي زر آخر بالصفحة.
  const staffAuditHidden = isCashier;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#212121] pb-24 md:pb-0 md:mr-[70px]">
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => !staffAuditHidden && router.push('/admin/settings/staff')}
            disabled={staffAuditHidden}
            className={`flex items-center justify-between px-4 py-4 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 transition-all ${staffAuditHidden ? 'opacity-50 cursor-not-allowed' : 'active:scale-95'}`}>
            <div className="flex items-center gap-2">
              <div className="w-9 h-9 rounded-xl bg-gray-100 dark:bg-slate-700 flex items-center justify-center">
            ...
            {staffAuditHidden ? <Lock size={16} className="text-gray-300 dark:text-slate-600" /> : <ChevronLeft size={16} className="text-gray-300 dark:text-slate-600" />}
          </button>
          <button onClick={() => !staffAuditHidden && router.push('/admin/audit')}
            disabled={staffAuditHidden}
            className={`flex items-center justify-between px-4 py-4 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 transition-all ${staffAuditHidden ? 'opacity-50 cursor-not-allowed' : 'active:scale-95'}`}>
            <div className="flex items-center gap-2">
              <div className="w-9 h-9 rounded-xl bg-gray-100 dark:bg-slate-700 flex items-center justify-center">
