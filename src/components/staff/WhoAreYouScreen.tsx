  ...
  if (selected && selected !== 'owner') {
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6" dir="rtl">
        ...
        <div className="flex gap-2.5 mb-8" dir="ltr">
          {[0, 1, 2, 3, 4, 5, 6, 7].map(i => (
            <span key={i} className={`w-3 h-3 rounded-full border-2 transition-all ${i < pin.length ? 'bg-[#2563eb] border-[#2563eb]' : 'border-slate-600'}`} />
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 w-full max-w-[280px]">
          {['1','2','3','4','5','6','7','8','9'].map(d => (
            <button key={d} onClick={() => handlePinChange(pin.length < 8 ? pin + d : pin)}
          ...
          <div />
          <button onClick={() => handlePinChange(pin.length < 8 ? pin + '0' : pin)} disabled={submitting || locked}
            className="h-16 rounded-2xl bg-slate-800 text-white text-2xl font-bold active:scale-95 transition-all disabled:opacity-40">
            0
          </button>
        ...
        <button onClick={() => submit(pin)} disabled={submitting || locked || pin.length < 4}
          className="w-full max-w-[280px] h-14 mt-4 rounded-2xl bg-[#2563eb] text-white font-bold text-lg active:scale-95 transition-all disabled:opacity-40 disabled:active:scale-100">
          دخول
        </button>
      </div>
    );
