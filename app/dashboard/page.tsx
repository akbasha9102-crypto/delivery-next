...
// رابط الداشبورد الموقع لتضمينه في رابط التوصيل
const DELIVERY_BASE = process.env.NEXT_PUBLIC_DELIVERY_BASE_URL ?? '';
...
export default function DriverDashboard() {
  ...
  // ── جلب الطلبات (مفلترة بالمطعم) ─────────────────────────────────────────

  const fetchIncoming = useCallback((s: Session) => {
    const rejected = getRejected(s.id);
    let q = supabase
      .from('orders')
    ...
    // فلتر المطعم — سائق مرتبط بمطعم يرى فقط طلباته
    if (s.restaurant_id) q = q.eq('restaurant_id', s.restaurant_id) as typeof q;
  ...
  useEffect(() => {
    ...
    fetchIncoming(session);
    fetchActive(session.id);
    fetchCompleted(session.id);

    const ch = supabase.channel(`driver-incoming-${session.restaurant_id ?? 'all'}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, () => {
        const s = sessionRef.current;
        if (s) { fetchIncoming(s); fetchActive(s.id); fetchCompleted(s.id); }
      })
      .subscribe();
  ...
  const acceptOrder = async (order: Order) => {
    ...
    const { data, error } = await supabase
      .from('orders')
      .update({ driver_id: session.id, driver_name: session.name, driver_phone: session.phone })
      .eq('id', order.id)
      .is('driver_id', null)
      .eq('status', 'preparing')
      .select('id')
  ...
  // رابط صفحة التوصيل — يفتح من الموقع الأصلي
        ...
        <div className="flex flex-col items-center">
          ...
          {session.restaurant_name && (
            <div className="flex items-center gap-1 mt-0.5">
              ...
              <p className="text-blue-300 text-xs font-bold">{session.restaurant_name}</p>
            </div>
          )}
        </div>
